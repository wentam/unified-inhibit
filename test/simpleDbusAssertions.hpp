...
static void simpleDbusAssertions(
  DBus& dbus,
  ...
  try {
    Quiet q;
    auto i1 = construct([](auto a, auto b){}, [](auto a, auto b){});
  } catch(...) { except = true; }
  ...
  try {
    Quiet q;
    auto i1 = construct([](auto a, auto b){}, [](auto a, auto b){});
    auto i2 = construct([](auto a, auto b){}, [](auto a, auto b){});
  } catch(...) { except = true; }
  ...
  for (int j = 0; j < 2; j++) {
    ...
    auto r = dbus.newMethodCall(dbusName.c_str(), dbusPath.c_str(),
                                dbusInterface.c_str(), "Inhibit")
    ...
    assert(inhibitExists, [&i, &appname, &reason, &inhibitType]() {
      Inhibit in;
    ...
    assert(validCookie && inhibitExists, [&dbus, &cookie, &dbusName, &dbusInterface, &dbusPath]() {
      bool noreply = false;
      bool null = false;
      try {
        auto r = dbus.newMethodCall(dbusName.c_str(), dbusPath.c_str(),
                                    dbusInterface.c_str(), "UnInhibit")
    ...
    assert(haveInhibit, [&i, &req, &inhibitType]() {
      Inhibit in;
      for (auto& [id, lin] : i->activeInhibits) { in = lin; break; }
    ...
    try {
      ...
      if (inhibitType == InhibitType::SUSPEND) req.type = InhibitType::SCREENSAVER;
      i->inhibit(req);
    ...
    // Application crash/sender dissappear
    {
...

      {
        ...
        const char* appname = "appname";
        const char* reason = "reason";
        auto r = dbus2.newMethodCall(dbusName.c_str(), dbusPath.c_str(),
                                      dbusInterface.c_str(), "Inhibit")
          .appendArgs(DBUS_TYPE_STRING, &appname, DBUS_TYPE_STRING, &reason, DBUS_TYPE_INVALID)
          ->sendAwait(200);
